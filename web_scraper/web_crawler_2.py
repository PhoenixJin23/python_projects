...
def scrape_books():
    ...
    for start_num in range(1,51):
        url=f"http://books.toscrape.com/catalogue/page-{start_num}.html"
        print(f"正在爬取网站第{start_num}页")
        try:
        ...
        except Exception as e: #如果出错了，抓所有类型的错误并存入e变量
            print(f"爬取第{start_num}页时出错：{e}")
            break
